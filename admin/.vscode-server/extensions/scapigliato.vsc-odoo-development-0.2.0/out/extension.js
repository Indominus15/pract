...
const odoo_lib = require("odoo-xmlrpc");
...
function create_model_table(odoo_class, odoo) {
    var inParams = [];
    ...
    odoo.execute_kw('ir.model', 'search_read', params, function (err, value) {
        if (err) {
        ...
        var params = [];
        params.push(inParams);
        odoo.execute_kw('ir.model.fields', 'read', params, function (err2, fields) {
            if (err2) {
                return;
            }
            // Show results
            const panel = vscode.window.createWebviewPanel('odoo.dev.model.result.' + odoo_class, 'Model:' + odoo_class, vscode.ViewColumn.One);
...
function activate(context) {
    ...
    let disposable_security_rule = vscode.commands.registerCommand('extension.dev.odoo.create.security.rule', () => {
        ...
        vscode.window.showInformationMessage('Odoo security rule created in clipboard for ' + odoo_class_dot);
    });
